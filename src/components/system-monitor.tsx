...
export function SystemMonitor({ sessionId }: SystemMonitorProps) {
    ...
    if (isLoading) {
        return (
            <Card className="border-b rounded-none shadow-none bg-card/30">
    ...
    if (!stats) {
        return (
            <Card className="border-b rounded-none shadow-none bg-destructive/10">
            ...
            <CardContent className="p-2">
                <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="flex flex-col items-center justify-center p-2 rounded-lg bg-background/50">
                        <h4 className="text-sm font-semibold flex items-center gap-1.5"><Cpu className="h-4 w-4 text-primary"/>CPU</h4>
                        <p className="text-2xl font-bold font-mono">{stats.cpu.usage.toFixed(1)}%</p>
                        <Progress value={stats.cpu.usage} className="h-1.5 mt-2" />
                    </div>
                    <div className="flex flex-col items-center justify-center p-2 rounded-lg bg-background/50">
                         <h4 className="text-sm font-semibold flex items-center gap-1.5"><MemoryStick className="h-4 w-4 text-primary"/>Memory</h4>
                         ...
                         <p className="text-xs text-muted-foreground mt-1">{stats.memory.used}MB / {stats.memory.total}MB</p>
                    </div>
                     <div className="flex flex-col items-center justify-center p-2 rounded-lg bg-background/50">
                         <h4 className="text-sm font-semibold flex items-center gap-1.5"><HardDrive className="h-4 w-4 text-primary"/>Disk</h4>
                          <div className="h-12 w-12">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                          ...
                          <p className="text-xs text-muted-foreground mt-1">{stats.disk.used.toFixed(1)}GB / {stats.disk.total.toFixed(1)}GB</p>
                    </div>
                </div>
            </CardContent>
        </Card>
